...
void VCA::set_adsr(float attack_ms,
                   float decay_ms,
                   float sustain,
                   float release_ms,
                   u32 sample_rate)
...
float VCA::get_amplitude(MessageStatus note_status,
                         u32 ticks_since_status_change)
{
    if (note_status == NoteOff)
    {
        if (ticks_since_status_change > release_ticks)
            return 0.0f;
        return interpolate(ticks_since_status_change, 0, release_ticks, sustain,
                           0.0f) *
               level;
    }
    ...
    if (ticks_since_status_change > attack_ticks)
        return interpolate(ticks_since_status_change - attack_ticks, 0,
                           decay_ticks, 1.0f, sustain) *
               level;

    return interpolate(ticks_since_status_change, 0, attack_ticks, 0, 1.0f) *
           level;
}

void VCA::set_level(float level)
{
    this->level = level;
}
